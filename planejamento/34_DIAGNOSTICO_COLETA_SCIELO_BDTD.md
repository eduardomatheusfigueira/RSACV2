# 34 — Diagnóstico da Coleta: SciELO e BDTD
...
> (§ 34.2). O que segue é o diagnóstico do código **depois** daquele ciclo.
...
mesmo orçamento de requisições por segundo e eliminar o tempo parado (§ 35.3).

---

## 34.2 O que está bom — e deve ser preservado

Ser justo com o código existente é parte do diagnóstico. O seguinte está certo
e o plano do doc 35 não deve tocá-lo:
...
- **Contrato de coleta explícito.** `HarvestQuery` / `RawPaperRecord` /
  ...
  registro e capacidade. É a peça que permite tudo que o doc 35 propõe.
...
- **Persistência fora do event loop.** `asyncio.to_thread(_persist_batch_sync, …)`
  ...
  registros/s, escala linear até 8.000** (§ 34.6). A deduplicação **não é** o
...
## 34.3 Velocidade — o diagnóstico medido
...
### 34.3.2 · O semáforo do BDTD é decorativo
...
### 34.3.3 · O SciELO passa 80% do tempo dormindo
...
### 34.3.4 · Página de 15 onde cabem 50
...
### 34.3.5 · Tudo é serial, exceto as fontes
...
### 34.3.6 · Botões de ritmo que não ligam em nada
...
## 34.4 Confiabilidade — onde a coleta mente

### 34.4.1 · Falha parcial vira sucesso silencioso
...
### 34.4.2 · A auditoria para na fonte, e o PRISMA precisa do descritor
...
Sem isso também não há retomada possível (§ 34.4.7): não se sabe onde parou.

### 34.4.3 · Corrida na deduplicação entre fontes
...
é justamente o que o doc 35 quer aumentar.

### 34.4.4 · A similaridade funde trabalhos distintos, e o DOI não a impede
...
### 34.4.5 · Política de retry sem princípio
...
### 34.4.6 · Dois acervos sob um nome só, e a raspagem apontando para o errado
...
### 34.4.7 · Sem retomada e sem orçamento de tempo
...
### 34.4.8 · O WebSocket empurra, e a coleta espera
...
## 34.5 Profissionalismo — as promessas que o código não cumpre

### 34.5.1 · Capacidades declaradas que ninguém lê
...
### 34.5.2 · O frontend não fecha o ciclo
...
### 34.5.3 · Os filtros não saem da tela
...
dos 4,8 registros/s do § 34.3.2.

### 34.5.4 · Detalhes de acabamento
...
### 34.5.5 · A suíte de testes não cobre nenhuma borda
...
| Correção dos dados | **Frágil** | Fusão indevida por similaridade e corrida na dedup (§ 34.4.3–4) |
| Confiabilidade | **Insuficiente** | Falha parcial vira sucesso silencioso (§ 34.4.1) |
| Rastreabilidade | **Insuficiente** | Sem contabilidade por descritor (§ 34.4.2) |
| Velocidade | **Insuficiente** | Concorrência real 1; 80% de espera ociosa (§ 34.3) |
| Cobertura de teste | **Insuficiente** | Só caminho feliz (§ 34.5.5) |
| Coerência com a UI | **Insuficiente** | Filtros inalcançáveis, fim de coleta não detectado (§ 34.5.2–3) |
...
ser testado nas bordas. É o que o doc 35 organiza.

---

## 34.8 Verificações que exigem rede

Itens que este diagnóstico **não** pôde fechar sem sair para a internet, e que
abrem a fase 0 do doc 35:

1. `count=50` no `search.scielo.org` devolve 50 itens? (§ 34.3.4)
...
4. Qual a taxa de requisições que cada base tolera antes de 429/403, para
   calibrar o governador de host (§ 35.3)?
