# 29 — Plano de Robustez e Velocidade da Coleta

> **Objetivo deste documento:** transformar os achados do doc 28 num plano
...
## 29.1 As quatro afirmações que o plano precisa tornar verdadeiras
...
## 29.2 Ordem, e por que ela não é negociável
...
- **Paralelismo amplifica a corrida da dedup** (doc 28 § 28.4.3). Subir a
  concorrência antes de fechar a escrita multiplica duplicatas que escapam.
- **Paralelismo sem governador de host vira 429**, e 429 mal tratado (§ 28.4.5)
...
## 29.3 O desenho de paralelismo

### 29.3.1 O princípio: gastar o mesmo, esperar menos
...
Orçamento inicial proposto, a calibrar com a Fase 0 (§ 29.4):
...
### 29.3.2 `HostGovernor` — um por host, compartilhado por todas as tarefas
...
- **A pausa é intervalo entre requisições, não sono depois da resposta.** Devolve
  os 80% de espera ociosa do SciELO (doc 28 § 28.3.3) sem tocar na taxa.
...
### 29.3.3 Três níveis de sobreposição, na ordem do retorno
...
41,3 s → ~10 s com K=4 no cenário do doc 28 § 28.3.1. **Requisito de ordem:**
...
`set` protegido, ou de ser feita no escritor único (§ 29.5.2) — preferível.
...
portal (estabilidade da ordenação sob paginação profunda — doc 28 § 28.8 item 2)
...
## 29.4 Fase 0 — Verdade e instrumento

**Entregas**

1. `backend/scripts/bench_coleta.py` — ✅ entregue com o doc 28; passa a ser o
...
3. **Sonda de rede** (`scripts/sondar_bases.py`, execução manual, fora da CI):
   responde às cinco perguntas do doc 28 § 28.8 — `count=50` no SciELO, teto de
   paginação profunda do VuFind, filtro `publishDate` com aspas, taxa tolerada
   antes de 429, necessidade do cookie `OasisbrVerify`.

**Aceite:** os números do § 29.3.1 deixam de ser proposta e passam a ser medidos.

---

## 29.5 Fase 1 — A coleta para de mentir

### 29.5.1 Contabilidade por descritor
...
### 29.5.2 `partial` deixa de virar `completed`
...
### 29.5.3 Capacidades param de mentir
...
## 29.6 Fase 2 — Escritor único e dedup que não perde estudo

### 29.6.1 Escritor único
...
Resolve de uma vez: a corrida do doc 28 § 28.4.3 (só um escritor), a contenção
...
### 29.6.2 A similaridade para de fundir o que é distinto
...
2. **Guarda de token discriminante.** Se os títulos normalizados diferem apenas
   ...
   medidos no doc 28 § 28.4.4 caem por esta regra.
...
## 29.7 Fase 3 — Ritmo e política de erro

- `HostGovernor` (§ 29.3.2) instanciado por host e injetado nos coletores.
...
- **Separar OasisBR do BDTD** (doc 28 § 28.4.6): fonte própria com `source_name`
...
## 29.8 Fase 4 — Paralelismo

Executa os três níveis do § 29.3.3 **nesta ordem**, medindo entre cada um:
...
página antes disso trunca todo descritor em 15 registros (doc 28 § 28.3.4).
...
## 29.9 Fase 5 — Retomada e orçamento de tempo
...
## 29.10 Fase 6 — A interface fecha o ciclo
...
2. `HarvestPage.tsx` trata `harvest_progress`, `harvest_source_failed`,
   ...
   todos (doc 28 § 28.5.2). Uma fonte que falha **precisa** aparecer na tela.
...
6. `ws_manager.broadcast` com fila limitada por conexão e descarte do mais
   antigo: o observador nunca freia a coleta (doc 28 § 28.4.8).

---

## 29.11 Fase 7 — Testes das bordas
...
| Falha definitiva ⇒ descritor `partial` ⇒ run `completed_with_errors` | o achado nº 1 do doc 28 |
| Cancelamento no meio de um lote | escrita depois do desfecho |
| Corrida de dedup com 4 fontes simultâneas | § 29.6.1 |
| Falsos positivos de similaridade (5 pares) | § 29.6.2 |
| Paridade série × paralelo por conjunto de `source_id` | § 29.8 |
...
uma correção:** § 29.5.2 (`partial` deixar de virar `completed`), porque é a que
