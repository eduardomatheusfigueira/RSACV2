...
from app.security.dependencies import require_local_token

# ── As duas metades da API ────────────────────────────────────────────
#
# `public_router` é a lista **completa** de exceções de §29.3.1 — e ela encolheu
# junto com o login: sobraram `health` e `auth/status`, as duas que precisam
# responder antes de a interface saber se tem credencial válida. Nada mais
# entra aqui.
#
# `api_router` exige o token local por dependência do próprio router, e não por
